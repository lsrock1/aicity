...
import pytorchvideo
from data import City, CityDetection
...
transform_train_detection = Compose(
            [
            ApplyTransformToKey(
              key="video",
              transform=Compose(
...
class DetectionModule(pytorch_lightning.LightningDataModule):
    ...
    def load_dataset_(self, split='train'):
        ...
        return CityDetection(
            info, 'train' if split == 'train' else 'val',
            pytorchvideo.data.make_clip_sampler("random", self._CLIP_DURATION),
            transform=transform_train_detection
        )
        # return LabeledVideoDataset(info, pytorchvideo.data.make_clip_sampler("random", self._CLIP_DURATION), decode_audio=False,  transform=)
